function dxdt = lys_leu_bilateral_cf_complete_model(t, x, D, G_fm, K_fm, L_fm, params)
...
% Vmax_dl_k:                    Maximum lysine uptake rate by the leucine auxotroph, mmol/h
% Vmax_dk_l:                    Maximum leucine uptake rate by the lysine axutroph, mmol/h
...
% phi_dl_k:                     Fraction of glucose uptake allocated to lysine production
% phi_dk_l:                     Fraction of glucose uptake allocated to leucine production
% varphi_dl_k:                  Lysine leakage fraction of the leucine auxotroph
% varphi_dk_l:                  Leucine leakage fraction of the lysine auxotroph
...
[Vmax_g,Km_g,Vmax_dk_k,Vmax_dl_k,Vmax_dk_l,Vmax_dl_l,Km_k,Km_l,delta_k,delta_l,...
 phi_dl_k,phi_dk_l,varphi_dl_k,varphi_dk_l,gamma_g,gamma_k,gamma_l,eta_dk,eta_dl] = params_cell{:};
         
% Glucose uptake rate
J_upt_dk_g = Vmax_g*Glu/(Km_g+Glu);
J_upt_dl_g = J_upt_dk_g;

% Leucine and lysine biosynthesis rate
J_syn_dk_l = J_upt_dk_g * delta_l * phi_dk_l;
J_syn_dl_k = J_upt_dl_g * delta_k * phi_dl_k;

% Leakage flux
J_leak_dk_l = J_syn_dk_l * varphi_dk_l;
J_leak_dl_k = J_syn_dl_k * varphi_dl_k;

% Amino acid uptake rate
J_upt_dk_k = Vmax_dk_k*Lys/(Km_k+Lys);
J_upt_dl_l = Vmax_dl_l*Leu/(Km_l+Leu);
J_upt_dk_l = Vmax_dk_l*Leu/(Km_l+Leu);
J_upt_dl_k = Vmax_dl_k*Lys/(Km_k+Lys);

% Specific growth rate
J_grow_dk = min([gamma_k*J_upt_dk_k, gamma_g*(1-phi_dk_l)*J_upt_dk_g, gamma_l*(J_upt_dk_l+J_syn_dk_l-J_leak_dk_l)]);
J_grow_dl = min([gamma_l*J_upt_dl_l, gamma_g*(1-phi_dl_k)*J_upt_dl_g, gamma_k*(J_upt_dl_k+J_syn_dl_k-J_leak_dl_k)]);
...
dxdt(6) = D*(K_fm-Lys)+(J_leak_dl_k*DeltaL_a-J_upt_dk_k*DeltaK_a-J_upt_dl_k*DeltaL_a); % lysine
dxdt(7) = D*(L_fm-Leu)+(J_leak_dk_l*DeltaK_a-J_upt_dl_l*DeltaL_a-J_upt_dk_l*DeltaK_a); % leucine
